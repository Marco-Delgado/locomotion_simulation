...
class ImagePoseListener:
    ...
    def subscribe_listener(self):
        self.sub_color_image = rospy.Subscriber(self.color_topic, msg_Image, self.ImageColorCallback)
        self.sub_image = rospy.Subscriber(self.depth_topic, msg_Image, self.imageDepthCallback)
...
def save_image_data(listener, image_dir=None):
    if image_dir is None:
        image_dir = os.path.join(os.getcwd(), datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S'))
    ...
    for i in range(len(listener.frames)):
        imageio.imsave(os.path.join(image_dir, 'depth_image_' + listener.frames[i] + '.png'))
        print('saved depth image')
        if i < len(listener.color_frames):
            imageio.imsave(os.path.join(image_dir, 'color_image_' + listener.color_frames[i] + '.png'))
